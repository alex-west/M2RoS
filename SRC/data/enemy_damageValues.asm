; This file was automatically generated from enemies.csv. Please do not edit this directly.
    db $08 ; Tsumari
    db $08 ; Tsumari
    db $08 ; Tsumari
    db $08 ; Tsumari
    db $10 ; Skreek
    db $10 ; Skreek
    db $10 ; Skreek
    db $10 ; Skreek
    db $03 ; Skreek projectile
    db $10 ; Drivel
    db $10 ; Drivel
    db $10 ; Drivel
    db $03 ; Drivel projectile
    db $03 ; Drivel projectile
    db $03 ; Drivel projectile
    db $03 ; Drivel projectile
    db $03 ; Drivel projectile
    db $03 ; Drivel projectile
    db $05 ; Small bugs
    db $05 ; Small bugs
    db $10 ; Hornoad
    db $10 ; Hornoad
    db $15 ; Senjoo
    db $08 ; Gawron
    db $08 ; Gawron
    db $00 ; Gawron spawner?
    db $00 ; Gawron spawner?
    db $12 ; Chute leech
    db $12 ; Chute leech
    db $12 ; Chute leech
    db $15 ; (uses same spritemap as 41h autrack)
    db $15 ; (uses same spritemap as 4Ah wallfire)
    db $10 ; Needler
    db $10 ; Needler
    db $10 ; Needler
    db $10 ; Needler
    db $00 ; (no graphics)
    db $00 ; (no graphics)
    db $00 ; (no graphics)
    db $00 ; (no graphics)
    db $11 ; Skorp
    db $11 ; Skorp
    db $11 ; Skorp
    db $11 ; Skorp
    db $10 ; Glow fly
    db $10 ; Glow fly
    db $10 ; Glow fly
    db $10 ; Glow fly
    db $13 ; Moheek
    db $13 ; Moheek
    db $13 ; Moheek
    db $13 ; Moheek
    db $00 ; Rock icicle
    db $08 ; Rock icicle
    db $08 ; Rock icicle
    db $08 ; Rock icicle
    db $12 ; Yumee
    db $12 ; Yumee
    db $12 ; Yumee
    db $12 ; Yumee
    db $00 ; Yumee spawner?
    db $00 ; Yumee spawner?
    db $20 ; Octroll
    db $20 ; Octroll
    db $20 ; Octroll
    db $15 ; Autrack
    db $15 ; Autrack
    db $15 ; Autrack
    db $15 ; Autrack
    db $10 ; Autrack projectile
    db $15 ; Autoad
    db $15 ; Autoad
    db $00 ; Sideways Autoad (unused)
    db $00 ; Sideways Autoad (unused)
    db $15 ; Wallfire
    db $15 ; Wallfire
    db $FF ; Wallfire
    db $10 ; Wallfire projectile
    db $10 ; Wallfire projectile
    db $10 ; Wallfire projectile
    db $10 ; Wallfire projectile
    db $15 ; Gunzoo
    db $15 ; Gunzoo
    db $15 ; Gunzoo
    db $08 ; Gunzoo diagonal projectile
    db $08 ; Gunzoo diagonal projectile
    db $08 ; Gunzoo diagonal projectile
    db $08 ; Gunzoo horizontal projectile
    db $08 ; Gunzoo horizontal projectile (unused frame)
    db $08 ; Gunzoo horizontal projectile
    db $08 ; Gunzoo horizontal projectile
    db $08 ; Gunzoo horizontal projectile
    db $15 ; Autom
    db $15 ; Autom
    db $10 ; Autom projectile
    db $10 ; Autom projectile
    db $10 ; Autom projectile
    db $10 ; Autom projectile
    db $10 ; Autom projectile
    db $15 ; Shirk
    db $15 ; Shirk
    db $FF ; Septogg
    db $FF ; Septogg
    db $20 ; Moto
    db $20 ; Moto
    db $20 ; Moto
    db $10 ; Halzyn
    db $20 ; Ramulken
    db $20 ; Ramulken
    db $00 ; Musical stinger event trigger
    db $FF ; (uses same spritemap as 72h proboscum)
    db $00 ; (no graphics)
    db $00 ; (no graphics)
    db $00 ; (no graphics)
    db $FF ; Proboscum
    db $00 ; Proboscum
    db $00 ; Proboscum
    db $FF ; Missile block
    db $20 ; Arachnus
    db $20 ; Arachnus
    db $20 ; Arachnus
    db $20 ; Arachnus
    db $20 ; Arachnus
    db $02 ; Arachnus projectile
    db $02 ; Arachnus projectile
    db $00 ; (no graphics)
    db $00 ; (no graphics)
    db $00 ; (no graphics)
    db $FF ; Plasma beam orb
    db $00 ; Plasma beam
    db $FF ; Ice beam orb (and bent neck of Queen’s vomiting pose!?)
    db $00 ; Ice beam
    db $FF ; Wave beam orb
    db $00 ; Wave beam
    db $FF ; Spazer beam orb
    db $00 ; Spazer beam
    db $FF ; Bombs orb
    db $00 ; Bombs
    db $FF ; Screw attack orb
    db $00 ; Screw attack
    db $FF ; Varia suit orb
    db $00 ; Varia suit
    db $FF ; Hi-jump boots orb
    db $00 ; Hi-jump boots
    db $FF ; Space jump orb
    db $00 ; Space jump
    db $FF ; (spider ball orb?)
    db $00 ; Spider ball
    db $FF ; (spring ball orb?)
    db $00 ; Spring ball
    db $FF ; (energy tank orb?)
    db $00 ; Energy tank
    db $FF ; (missile tank orb?)
    db $00 ; Missile tank
    db $30 ; Blob thrower (sprite is written to WRAM)
    db $00 ; Energy refill
    db $FF ; Arachnus orb
    db $00 ; Missile refill
    db $10 ; Blob thrower projectile
    db $10 ; Blob thrower projectile
    db $FE ; Metroid
    db $00 ; Metroid hatching
    db $00 ; (no graphics)
    db $10 ; Alpha metroid
    db $10 ; Alpha metroid
    db $FF ; Baby metroid egg
    db $FF ; Baby metroid egg
    db $FF ; Baby metroid egg
    db $00 ; Baby metroid
    db $00 ; Baby metroid
    db $00 ; (no graphics)
    db $00 ; (no graphics)
    db $00 ; (no graphics)
    db $15 ; Gamma metroid
    db $15 ; Gamma metroid projectile
    db $15 ; Gamma metroid projectile
    db $15 ; Gamma metroid
    db $00 ; (no graphics)
    db $00 ; Gamma metroid shell
    db $20 ; Zeta metroid hatching
    db $20 ; Zeta metroid
    db $20 ; Zeta metroid
    db $20 ; Zeta metroid
    db $20 ; Zeta metroid
    db $20 ; Zeta metroid
    db $20 ; Zeta metroid
    db $00 ; Zeta metroid
    db $00 ; Zeta metroid
    db $00 ; Zeta metroid
    db $00 ; Zeta metroid
    db $10 ; Zeta metroid projectile
    db $25 ; Omega metroid
    db $25 ; Omega metroid
    db $25 ; Omega metroid
    db $25 ; Omega metroid
    db $25 ; Omega metroid
    db $00 ; Omega metroid
    db $00 ; Omega metroid
    db $12 ; Omega metroid projectile
    db $12 ; Omega metroid projectile
    db $12 ; Omega metroid projectile
    db $12 ; Omega metroid projectile
    db $12 ; Omega metroid projectile
    db $12 ; Omega metroid projectile
    db $12 ; Omega metroid projectile
    db $12 ; (omega metroid projectile?)
    db $FE ; Metroid
    db $FE ; Metroid (hurt)
    db $FF ; Flitt
    db $FF ; Flitt
    db $00 ; Stalagtite (unused)
    db $10 ; Gravitt
    db $10 ; Gravitt
    db $10 ; Gravitt
    db $10 ; Gravitt
    db $10 ; Gravitt
    db $12 ; Gullugg
    db $12 ; Gullugg
    db $12 ; Gullugg
    db $00 ; Baby metroid egg preview
    db $00 ; (no graphics)
    db $00 ; (no graphics)
    db $00 ; (no graphics)
    db $00 ; (no graphics)
    db $00 ; Small health drop
    db $00 ; Small health drop
    db $00 ; Metroid death / missile door / screw attack explosion
    db $00 ; Metroid death / missile door / screw attack explosion
    db $00 ; Metroid death / missile door / screw attack explosion
    db $00 ; Metroid death / missile door / screw attack explosion
    db $00 ; Metroid death / missile door / screw attack explosion
    db $00 ; Metroid death / missile door / screw attack explosion
    db $00 ; Enemy death explosion
    db $00 ; Enemy death explosion
    db $00 ; Enemy death explosion
    db $00 ; Enemy death explosion (extra frame for enemies not dropping small health)
    db $00 ; Big energy drop
    db $00 ; Big energy drop
    db $00 ; Missile drop
    db $00 ; Missile drop
    db $40 ; Metroid Queen neck (no graphics)
    db $40 ; Metroid Queen head left half (no graphics)
    db $20 ; Metroid Queen projectile/head right half (no graphics)
    db $40 ; Metroid Queen body (no graphics)
    db $00 ; (no graphics)
    db $40 ; Metroid Queen mouth closed (no graphics)
    db $40 ; Metroid Queen mouth open (no graphics)
    db $FF ; Metroid Queen mouth stunned (no graphics)
    db $FF ; Missile door
    db $FF ; (no graphics)
    db $FF ; (no graphics)
    db $00 ; (no graphics)
    db $00 ; (no graphics)
    db $00 ; Nothing - flitt (no graphics)
    db $00 ; ?
